...
void LoadTextureData(ObjData* objData) 
{
	...
	//std::cout << "MATERIAL COUNT: " << objData->materials.size() << std::endl;
...
void LoadObjToMemory(ObjData* objData, GLfloat scaleFactor, GLfloat tOffset[], std::string objName) {
	...
	//Generate Normals if no normal data exists in model
	if (objData->attrib.normals.size() == 0)
	{
		for (int i = 0; i < vertexList.size() / 3; i++)
		{
			...
			vertexList[idx].normal = vertexList[idx + 1].normal = vertexList[idx + 2].normal = normal;
		}
	}
